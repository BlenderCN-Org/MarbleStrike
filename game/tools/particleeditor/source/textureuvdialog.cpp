...
{
	wxImage tgaImage;
	tgaImage.LoadFile( path, wxBITMAP_TYPE_TGA );
	if ( tgaImage.IsOk() )
	{
		int width = tgaImage.GetWidth();
		int height = tgaImage.GetHeight();

		mImage = wxBitmap( tgaImage );
				
		width = Math::Max<int>( width, 500 );
		height = Math::Max<int>( height, 500 );		

		mImagePanel->SetMinSize( wxSize( width, height ) );
		Fit();	
	}	
}
	...
	if (gc)
	{
		...
		{	
			gc->DrawBitmap( 
				mImage, 
				mPanelPosition[0], 
				-textureHeight/mScale * ( mScale - 1 ) - mPanelPosition[1], 
				textureWidth, 
				textureHeight 
				);
		}
		...
		gc->DrawRectangle( 
			...
			( textureHeight - mCurrentUVPosition[1] - mCurrentUVSize[1] ) -textureHeight/mScale * ( mScale - 1 ) - mPanelPosition[1],
			mCurrentUVSize[0],
			mCurrentUVSize[1]
		);
		...
		for ( int i = 0; i < 4; i++ )
		{
			...
			gc->DrawEllipse(
				...
				( textureHeight - boundingBoxVertices[i][1] - height ) -textureHeight/mScale * ( mScale - 1 ) - mPanelPosition[1] + height/2.0f,
				width,
				height
				);
		}
