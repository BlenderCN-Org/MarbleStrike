...
OGLESBlendState::OGLESBlendState() 
{    
}
...
{		
		...
		)
	{
		...
		{
			OGLES::oglEnable( GL_BLEND );
			OGLES::oglBlendFunc( gBlendTranslation[mData.mSrcBlend], gBlendTranslation[mData.mDestBlend] );

			//OGLES::oglBlendFuncSeparate( 
		...
		)
	{
		...
		{
			//OGLES::oglEnable( GL_ALPHA_TEST );
			//float alphaRef = static_cast<float>( mData.mAlphaRef ) / 255.0f;
			//OGLES::oglAlphaFunc( gAlphaFuncComparisonTranslation[mData.mAlphaFunc], alphaRef );
		}
		else
		{
			//OGLES::oglDisable( GL_ALPHA_TEST );
		}
	}
